...
function launchsentinel() {
  ...
  while true; do
    ...
    MASTER_IP=$(kubectl get pod -o jsonpath='{range .items[*]}{.metadata.name} {..podIP} {.status.phase}{"\n"}{end}' -l redis-role=master|grep Running|grep $REDIS_CHART_PREFIX|awk '{print $2}'|xargs)
